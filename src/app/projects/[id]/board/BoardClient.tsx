...
import type { Act, Scene } from "@prisma/client";
import { Modal } from "@/components/Modal";
import { IconPlus, IconTrash, IconEdit } from "@/components/icons";
import { deleteAct, deleteScene, moveScene, saveAct, saveScene } from "./actions";
...
export function BoardClient({
  ...
  const columns = [
    ...acts.map((a) => ({ id: a.id, title: a.title, act: a })),
    { id: UNFILED, title: "미분류", act: null as Act | null },
  ];

  const scenesOf = (colId: string) =>
    scenes.filter((s) =>
      colId === UNFILED ? s.actId === null : s.actId === colId
    );
  ...
  return (
    <div className="flex h-[calc(100vh-0px)] flex-col px-5 py-8 md:h-screen">
      <header className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-ink">콘티 보드</h1>
          <p className="mt-1 text-sm text-ink-muted">
            장면 카드를 드래그해 흐름을 짜보세요. 장면 {scenes.length}개
          </p>
        </div>
        <button className="btn-outline" onClick={() => setActModal("new")}>
          <IconPlus width={16} height={16} /> 막 추가
        </button>
      </header>
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-1 flex-col gap-2 overflow-y-auto px-2.5 pb-2">
                ...
                >
                  <IconPlus width={14} height={14} /> 장면
                </button>
              </div>
            </div>
          );
...
function DeleteActButton({ projectId, act }: { projectId: string; act: Act }) {
  ...
  return (
    <>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="막 삭제" width="max-w-sm">
        <p className="text-sm text-ink-soft">
          <b>{act.title}</b> 막을 삭제합니다. 안에 있던 장면은 <b>미분류</b>로
          이동해요.
        </p>
...
function SceneModal({
  ...
  return (
    <Modal open onClose={onClose} title={scene ? "장면 편집" : "새 장면"} width="max-w-lg">
      <form
        action={async (fd) => {
      ...
      >
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="이 장면에서 무슨 일이 일어나는지, 어떤 감정선인지"
          />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="label">막</label>
            <select
              name="actId"
              defaultValue={scene?.actId ?? defaultActId ?? ""}
              className="input"
            >
...
function ActModal({
  ...
  return (
    <Modal open onClose={onClose} title={act ? "막 이름 수정" : "새 막"} width="max-w-sm">
      <form
        action={async (fd) => {
      ...
      >
        ...
        <div>
          <label className="label">막 이름 *</label>
          <input
            name="title"
            required
            autoFocus
